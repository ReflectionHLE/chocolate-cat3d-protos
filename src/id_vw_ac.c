...
uint8_t plotpixels[] = {0xC0, 0x30, 0x0C, 0x03};
uint8_t colorbyte[]  = {0x00, 0x55, 0xAA, 0xFF};
uint16_t colorword[] = {0x0000, 0x5555, 0xAAAA, 0xFFFF};
...
{
	uint8_t *destPtr = &screenseg[bufferofs+ylookup[y]+(x>>2)]; // byte on screen
	uint8_t maskOn = (colorbyte[color] & plotpixels[x&3]);
	uint8_t maskOff = ~plotpixels[x&3]; // mask off other pixels
...
{
	uint8_t *destPtr = &screenseg[bufferofs+ylookup[yl]+(x>>2)]; // byte on screen
	uint8_t maskOn = (colorbyte[color] & plotpixels[x&3]);
	uint8_t maskOff = ~plotpixels[x&3]; // mask off other pixels
	for (uint16_t loopVar = yh-yl+1; loopVar; --loopVar, destPtr += linewidth)
	{
...
{
	uint8_t *destPtr = &screenseg[bufferofs+xcoord+ylookup[ycoord]];
	uint8_t *tilePtr = grsegs[STARTTILE8]+(tile<<4);
...
{
	uint8_t *srcPtr = &segm[ofs];
	uint8_t *destPtr = &screenseg[dest];
	...
	for (uint16_t lineCounter = height, colCounter; lineCounter; --lineCounter, destPtr += linedelta)
	{
...
{
	uint8_t *srcPtr = &screenseg[source];
	uint8_t *destPtr = &screenseg[dest];
	for (uint16_t lineCounter = height; lineCounter; --lineCounter, srcPtr += linewidth, destPtr += linewidth) {
	...
	id0_unsigned_t wide,id0_unsigned_t height)
{
	...
	for (uint16_t lineCounter = height; lineCounter; --lineCounter, srcPtr += wide, destPtr += linewidth) {
		memcpy(destPtr, srcPtr, wide);
	}
}
	...
	id0_unsigned_t wide, id0_unsigned_t height)
{
	...
	for (uint16_t lineCounter = height; lineCounter; --lineCounter, srcPtr += linewidth, destPtr += wide) {
		memcpy(destPtr, srcPtr, wide);
	}
}
...
uint8_t fontcolor = 15; // 0-15 mapmask value
...
//uint16_t propchar; // the character number to shift // UNUSED VARIABLE
char *stringptr;

uint16_t fontcolormask; // font color expands into this

#define BUFFWIDTH 100
#define BUFFHEIGHT 32 // must be twice as high as font for masked fonts

uint8_t databuffer[BUFFWIDTH*BUFFHEIGHT];

uint16_t bufferwidth; // bytes with valid info / line
uint16_t bufferheight; // number of lines currently used

uint16_t bufferbyte;
uint16_t bufferbit;

uint16_t screenspot; // where the buffer is going

uint16_t bufferextra; // add at end of a line copy
uint16_t screenextra;
...
void VWL_XORBuffer(uint8_t *buffer)
{
	fontcolormask = colorword[fontcolor];
	uint8_t *srcPtr = buffer;
	uint8_t *destPtr = &screenseg[screenspot];
	uint16_t modbuffwidth = bufferwidth;
	if (!modbuffwidth)
	{
	...
	if (modbuffwidth & 1) // odd
	{
		...
		for (uint16_t line = 0; line < BUFFHEIGHT; ++line) {
	...
	modbuffwidth >>= 1;
	for (uint16_t lineCounter = bufferheight, colPairCounter; lineCounter; --lineCounter, srcPtr += bufferextra, destPtr += screenextra)
	{
		for (colPairCounter = modbuffwidth; colPairCounter; --colPairCounter, srcPtr += 2, destPtr += 2)
		{
			*(uint16_t *)destPtr ^= ((*(uint16_t *)srcPtr) & fontcolormask);
		}
	}
}
...
void ShiftPropChar(uint16_t charnum)
{
 ...
 */
	...
	uint8_t *destPtr = &databuffer[bufferbyte];
	uint16_t oldbufferbit = bufferbit;
	...
	case 1: // one byte character
		for (uint16_t loopvar = currfont->height; loopvar; --loopvar, destPtr += BUFFWIDTH-1)
		{
			SHIFTNOXOR
		}
		break;
	case 2: // two byte character
		for (uint16_t loopvar = currfont->height; loopvar; --loopvar, destPtr += BUFFWIDTH-2)
		{
			SHIFTNOXOR
			SHIFTNOXOR
		}
	...
	case 3: // three byte character
		for (uint16_t loopvar = currfont->height; loopvar; --loopvar, destPtr += BUFFWIDTH-3)
		{
			SHIFTNOXOR
			SHIFTNOXOR
			SHIFTNOXOR
	...
	case 4: // four byte character
		for (uint16_t loopvar = currfont->height; loopvar; --loopvar, destPtr += BUFFWIDTH-4)
		{
			SHIFTNOXOR
			SHIFTNOXOR
			SHIFTNOXOR
	...
	case 5: // five byte character
		for (uint16_t loopvar = currfont->height; loopvar; --loopvar, destPtr += BUFFWIDTH-5)
		{
			SHIFTNOXOR
			SHIFTNOXOR
			SHIFTNOXOR
	...
	case 6: // six byte character
		for (uint16_t loopvar = currfont->height; loopvar; --loopvar, destPtr += BUFFWIDTH-6)
		{
			SHIFTNOXOR
			SHIFTNOXOR
			SHIFTNOXOR
...
void VW_DrawPropString (id0_char_t id0_far *string)
{
	...
	do
	{
		uint8_t currChAsUnsigned = *(stringPtr++);
		if (!currChAsUnsigned)
		{
			break;
		}
