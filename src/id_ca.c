...
{
	// TODO (Chocolate Cat3D): Implement? (O_TEXT on Unix)
#if 0
	unlink ("DEBUG.TXT");
...
void CAL_CarmackExpand (id0_unsigned_t id0_far *source, id0_unsigned_t id0_far *dest, id0_unsigned_t length)
{
	...
	while (length)
	{
		...
		if (chhigh == NEARTAG)
		{
			count = ch&0xff;
			if (!count)
			{				// have to insert a 16-bit word containing the tag byte
				ch |= *((id0_unsigned_char_t id0_far *)inptr);
				inptr = (id0_unsigned_t id0_far *)((id0_unsigned_char_t id0_far *)inptr+1);
				//ch |= *((id0_unsigned_char_t id0_far *)inptr)++;
				*outptr++ = ch;
				length--;
			}
